steal('can/util', 'can/model', './underscore.js', function (can) {
	...
	var Model = can.Model({}, {
		setup : function (attributes, options) {
			...
			if (this.defaults) {
				defaults = _result(this, 'defaults');
			} else if(this.constructor.defaults) {
				defaults = _result(this.constructor, 'defaults');
			}

			attrs = _.extend({}, defaults, attributes || {});

			if (options && options.parse) {
				attrs = this.parse(attrs);
			}

			this.bind('change', function (ev, attr, how, to, from) {
			...
			args = [attrs, options];
			this._previousAttributes = _.extend({}, attrs);
		...
		change : function(options) {
			can.trigger(this, 'change', this, options);
			return this;
		},
